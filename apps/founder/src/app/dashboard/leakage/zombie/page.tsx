...
import { Users, Phone, IndianRupee, Clock, Settings } from "lucide-react";
import { PageHeader } from "@/components/layout/PageHeader";
import { Card } from "@/components/ui/Card";
import { Button } from "@/components/ui/Button";
import { StatCard } from "@/components/ui/StatCard";
import { Modal } from "@/components/ui/Modal";
import { zombieLeads } from "@/lib/mock-data";
  ...
  return (
    <div className="pb-10">
      <PageHeader
        ...
        description="6+ attempts · 14+ days · no positive signal — consuming telecaller capacity"
      />

      <div className="mt-4 grid grid-cols-2 gap-4 px-4 sm:px-6 lg:px-8 lg:grid-cols-4">
        <StatCard label="Zombie Leads" value="5" tone="danger" icon={Users} />
        <StatCard label="Total Calls Wasted" value="37" icon={Phone} />
        <StatCard label="Cost (₹40/call)" value="₹1,480" tone="danger" icon={IndianRupee} />
        <StatCard label="Avg Days in Funnel" value="20d" icon={Clock} />
      </div>

      <div className="mt-4 px-4 sm:px-6 lg:px-8">
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white px-4 py-3">
          <p className="text-sm text-slate-600">
            <span className="font-semibold text-red-600">5 zombie leads</span> have consumed{" "}
            <span className="font-semibold text-slate-900">37 calls</span> and{" "}
            <span className="font-semibold text-red-600">₹1,480</span> in telecaller time with zero conversion
            signal.
            <br />
          ...
          <table className="w-full min-w-[640px] text-sm">
            <thead>
              <tr className="border-b border-slate-100 text-left text-[11px] font-semibold uppercase tracking-wide text-slate-400">
                ...
                <th className="px-3 py-3 text-right">Days in Funnel</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {zombieLeads.map((z) => (
                <tr key={z.name}>
                  ...
                  <td className="px-3 py-3 text-blue-600">{z.source}</td>
                  <td className="px-3 py-3 text-right font-mono font-semibold text-amber-600">{z.calls}</td>
                  <td className="px-3 py-3 text-right font-mono font-semibold text-red-600">{z.days}d</td>
                  <td className="px-3 py-3 font-mono text-slate-400">{z.outcome}</td>
